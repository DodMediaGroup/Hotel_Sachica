...
class DescubrirController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if(isset($_POST['ItemDescubrir']))
        {
            $errors = false;

            $model->attributes=$_POST['ItemDescubrir'];
            ...
            if($model->validate(null, false)){
                ...
                if(!$errors && $model->save()){
                    $this->redirect(array('view','id'=>$model->iditem_des));
                }
            }
        }
    ...
    public function actionUpdate($id)
    {
     ...
     public function actionStatus($id){
        $itemDes = $this->loadModel($id);
        if($itemDes->status == 1)
            $itemDes->status = 0;
        else if($itemDes->status == 0)
            $itemDes->status = 1;
        else
    ...
    {
        $dataProvider=new CActiveDataProvider('Descubrir');
    ...
    public function actionAdmin()
    {
        ...
        $items_des = ItemDescubrir::model()->findAll(array('condition'=>'t.status != 2', 'order'=>'t.iditem_des DESC'));
    ...
    {
         $model=ItemDescubrir::model()->findByAttributes(array('iditem_des'=>$id), array('condition'=>'t.status != 2'));
        if($model===null)
            throw new CHttpException(404,'The requested page does not exist.');
        return $model;
    }
    ...
    {
        if(isset($_POST['ajax']) && $_POST['ajax']==='des-form')
        {
